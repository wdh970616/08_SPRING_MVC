 ...
 * 클래스 레벨에 공통 부분을 이용해 URL을 설정하고나면 매번 핸들러 메소드에 중복되는 URL 내용을 작성하지 않아도 된다.
 * 이 때, 와일드 카드를 이용해서 좀더 포괄적인 URL 패턴을 설정할 수 있다.
 */
@Controller
...
public class ClassMappingTestController {
    ...
    public String modifyAndDelete(Model model) {
        ...
        model.addAttribute("message", "POST 방식의 주문정보 수정과 삭제 공통 처리용 핸들러 메소드 호출함...");
    ...
    /*
     * path variable
     * @PathVariable 어노테이션을 이용해 요청 path로부터 변수를 받아올수 있다.
     * path variable로 전달되는 {변수명}값은 반드시 매개변수명이랑 같아야한다.
